          ...
          <div className="bg-black/70 rounded-lg px-4 py-2 text-center">
            ...
            <div className="text-xs text-gray-400 font-mono">
              {matchPhase === "active" && round > 0 ? (
                programStep > 0 ? (
                  <span className="text-yellow-400">TURN {round} — STEP {programStep}/3</span>
                ) : (
                  <span className="text-cyan-400 animate-pulse">TURN {round} — PLANNING...</span>
                )
              ) : (
                <>TICK {displayTick}</>
              )}
          ...
          {showFullHUD && (
            <div className="bg-black/70 rounded-lg px-3 py-2">
              <span className={`text-xs font-mono font-bold ${
                ...
                {matchPhase === "active" && (programStep > 0 ? "EXECUTING" : "MIND GAMES")}
      ...
      {showFullHUD && (
        <div className="flex justify-between px-8 mt-2">
          <div className="bg-blue-600/60 rounded-lg px-4 py-2">
            ...
            {programStep > 0 && currentMoves?.A && (
              <div className="text-xs font-mono font-bold text-yellow-300 mt-1">{currentMoves.A}</div>
            )}
          </div>
          <div className="bg-red-600/60 rounded-lg px-4 py-2">
            ...
            {programStep > 0 && currentMoves?.B && (
              <div className="text-xs font-mono font-bold text-yellow-300 mt-1">{currentMoves.B}</div>
            )}
          </div>
        </div>
      )}
