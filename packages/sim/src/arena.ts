...
import { ARENA_RADIUS, ARENA_FLOOR_Y } from "@ai-arena/protocol";
...
export class Arena {
  ...
  private createFloor(): RAPIER.RigidBody {
    // Static rigid body for the floor — sits just below y=0
    const bodyDesc = RAPIER.RigidBodyDesc.fixed()
      .setTranslation(0, ARENA_FLOOR_Y - 0.1, 0);
    const body = this.world.createRigidBody(bodyDesc);

    // Cylinder collider: (half_height, radius)
