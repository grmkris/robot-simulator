...
export class RobotFactory {
  ...
  create(id: AgentId, spawnX: number, spawnZ: number, facingAngleY: number | undefined, config: RobotConfig): Robot {
    ...
    chassis.setLinearDamping(2.0); // high damping for responsive, controllable movement
 ...
 */
export function applyMovementAction(robot: Robot, action: AgentAction): void {
...
export function getFacingDirection(chassis: RAPIER.RigidBody): [number, number] {
  const rot = chassis.rotation();
  // Forward = local +Z rotated by quaternion
  const fw_x = 2 * (rot.x * rot.z + rot.w * rot.y);
  const fw_z = 1 - 2 * (rot.x * rot.x + rot.y * rot.y);
  const len = Math.hypot(fw_x, fw_z);
  if (len < 0.001) return [0, 1]; // fallback to +Z
  return [fw_x / len, fw_z / len];
}

/**
 * Apply forward/backward drive via direct velocity control (post-physics).
 * Sets forward speed directly to target. Lateral velocity (from knockback)
 * is preserved with decay, giving collisions a satisfying push effect.
 */
function applyAgentDrive(robot: Robot, normalizedForce: number): void {
  ...
  robot.chassis.setLinvel(
    new RAPIER.Vector3(newVx, vel.y, newVz),
    true
  );
}

/**
 * Apply yaw rotation via direct angular velocity control (post-physics).
